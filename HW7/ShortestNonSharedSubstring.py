#ShortestNonSharedSubstring Implementation
...
def main():
    string_one, string_two = open_file("./data/rosalind_data_p67.txt")
    ...
    max_sub = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
    all_substrings = []
    for i in range(len(string_one)):
        for j in range(i):
            all_substrings.append(string_one[j:i])
    for sub in all_substrings:
        curr_node = 0
        count = 0
        for s in sub:
            count += 1
            v = [v for v in suffix_trie_two[curr_node] if v[1] == s]
            if not v:
                if count == len(sub):
