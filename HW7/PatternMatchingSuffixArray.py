#PatternMatchingSuffixArray Implementation
...
def search_for_pattern(pat, text, array):
    starting_postions = []
    for p in pat:
        length = len(p)
        for a in array:
            if text[a:a+length] == p:
                starting_postions.append(a)
    return starting_postions


def main():
    string, patterns = open_file("./data/rosalind_data_p69.txt")
    print(patterns)
    ...
    starting_positions = search_for_pattern(patterns,string,suffix_array)
