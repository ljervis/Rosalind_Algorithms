# FrequentWordsWithMismatchesAndReverseCompliments Implementation

import sys

dnaFile = open("rosalind_data_p10.txt")
...
def freqWithMis(seq, k, d):
	...
	for i in range(0, len(seq)-int(k)):
		seq1 = seq[i:i+int(k)]
		revSeq1 = RevComp(seq1)
		n1 = neighbors(seq1, int(d))
		n2 = neighbors(revSeq1, int(d))
		for x in n1:
	...
	for x in range(0, len(close)):
		if close[x] == 1:
			...
			revPat = RevComp(pat)
			freqArr[x] = approxCount(pat, seq, int(d)) + approxCount(revPat, seq, int(d))
